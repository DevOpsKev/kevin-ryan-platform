# Spec 5: Observability Stack (Grafana + Loki + Promtail)

## Task

1. Save this spec to `.spec/spec-5-observability-stack.md` in the repo (create the `.spec/` directory if it does not exist).
2. Implement all Terraform and Kubernetes manifest changes described below.
3. After completing all work, create a provenance record at `.provenance/spec-5-observability-stack.provenance.md` (create the `.provenance/` directory if it does not exist). See the **Provenance Record** section for the required format.

## Prerequisites

- Spec 2 deployed: PostgreSQL Flexible Server running with `grafana_db` database created
...
- **Grafana DB:** `grafana_db` on `psql-kevinryan-io.postgres.database.azure.com` (already created in Spec 2)
...
**Note:** `special = false` — learned from Spec 4 that special characters in passwords cause URL/config parsing issues.

### No other Terraform changes

No changes to variables, outputs, or modules. The `pgcrypto` extension is already allowlisted from Spec 4.
...
After completing the work, create `.provenance/spec-5-observability-stack.provenance.md` with the following structure:
...
1. This spec has been saved to `.spec/spec-5-observability-stack.md`
...
13. `pnpm lint` passes
14. The provenance record exists at `.provenance/spec-5-observability-stack.provenance.md` and contains all required sections
