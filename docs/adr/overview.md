...
| [007](./adr-007-postgresql-azure-flexible-server) | PostgreSQL on Azure Flexible Server | Accepted |
...
But fix the frontmatter first — that's what's actually breaking the build. You'll need to add `title:` to every ADR file. Want me to write a quick Claude Code prompt that adds frontmatter to all 15?